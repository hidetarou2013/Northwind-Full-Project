...
alter table nw_cities
  add constraint nw_cities_FK1 foreign key (region) references nw_regions(region_id)
  on delete cascade
  on update cascade;

alter table nw_customer_orders
  add constraint nw_customer_orders_FK1 foreign key (city) references nw_cities(city_id)
  on delete cascade
  on update cascade;

alter table nw_customer_orders
  add constraint nw_customer_orders_FK2 foreign key (country) references nw_countries(country_id)
  on delete cascade
  on update cascade;

alter table nw_customer_orders
  add constraint nw_customer_orders_FK3 foreign key (customer) references nw_customers(party_id)
  on delete cascade
  on update cascade;

alter table nw_customer_orders
  add constraint nw_customer_orders_FK4 foreign key (employee) references nw_employees(party_id)
  on delete cascade
  on update cascade;

alter table nw_customer_orders
  add constraint nw_customer_orders_FK5 foreign key (region) references nw_regions(region_id)
  on delete cascade
  on update cascade;

alter table nw_customer_orders
  add constraint nw_customer_orders_FK6 foreign key (shipper) references nw_shippers(shipper_id)
  on delete cascade
  on update cascade;

alter table nw_customers
  add constraint nw_customers_FK1 foreign key (party_id) references nw_parties(party_id)
  on delete cascade
  on update cascade;

alter table nw_employees
  add constraint nw_employees_FK1 foreign key (party_id) references nw_parties(party_id)
  on delete cascade
  on update cascade;

alter table nw_order_details
  add constraint nw_order_details_FK1 foreign key (customer_order) references nw_customer_orders(customer_order_id)
  on delete cascade
  on update cascade;

alter table nw_order_details
  add constraint nw_order_details_FK2 foreign key (product) references nw_products(product_id)
  on delete cascade
  on update cascade;

alter table nw_parties
  add constraint nw_parties_FK1 foreign key (city) references nw_cities(city_id)
  on delete cascade
  on update cascade;

alter table nw_parties
  add constraint nw_parties_FK2 foreign key (country) references nw_countries(country_id)
  on delete cascade
  on update cascade;

alter table nw_parties
  add constraint nw_parties_FK3 foreign key (region) references nw_regions(region_id)
  on delete cascade
  on update cascade;

alter table nw_products
  add constraint nw_products_FK1 foreign key (category) references nw_categories(category_id)
  on delete cascade
  on update cascade;

alter table nw_products
  add constraint nw_products_FK2 foreign key (supplier) references nw_suppliers(supplier_id)
  on delete cascade
  on update cascade;

alter table nw_purchase_orders
  add constraint nw_purchase_orders_FK1 foreign key (employee) references nw_employees(party_id)
  on delete cascade
  on update cascade;

alter table nw_purchase_orders
  add constraint nw_purchase_orders_FK2 foreign key (product) references nw_products(product_id)
  on delete cascade
  on update cascade;

alter table nw_regions
  add constraint nw_regions_FK1 foreign key (country) references nw_countries(country_id)
  on delete cascade
  on update cascade;

alter table nw_stores
  add constraint nw_stores_FK1 foreign key (city) references nw_cities(city_id)
  on delete cascade
  on update cascade;

alter table nw_stores
  add constraint nw_stores_FK2 foreign key (country) references nw_countries(country_id)
  on delete cascade
  on update cascade;

alter table nw_stores
  add constraint nw_stores_FK3 foreign key (region) references nw_regions(region_id)
  on delete cascade
  on update cascade;

alter table nw_suppliers
  add constraint nw_suppliers_FK1 foreign key (city) references nw_cities(city_id)
  on delete cascade
  on update cascade;

alter table nw_suppliers
  add constraint nw_suppliers_FK2 foreign key (country) references nw_countries(country_id)
  on delete cascade
  on update cascade;

alter table nw_suppliers
  add constraint nw_suppliers_FK3 foreign key (region) references nw_regions(region_id)
  on delete cascade
  on update cascade;

alter table user_login_role
  add constraint user_login_role_FK1 foreign key (login_role) references login_role(login_role_id)
  on delete cascade
  on update cascade;

alter table user_login_role
  add constraint user_login_role_FK2 foreign key (user_login) references user_login(user_login_id)
  on delete cascade
  on update cascade;
